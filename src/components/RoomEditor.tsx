...
export default function RoomEditor({ room, onDataChange, onGoHome }: { room: any, onDataChange: () => Promise<void>, onGoHome?: () => void }) {
  ...
  // 3. ฟังก์ชันส่งออก CSV
  const handleExportCSV = () => {
    if (bookings.length === 0) {
      alert('ไม่มีข้อมูลผู้จองให้ส่งออกครับ');
      return;
    }

    const headers = ['ลำดับ', 'โต๊ะ', 'ชื่อผู้จอง', 'วันที่จอง'];
    ...
    link.setAttribute("download", `รายชื่อผู้จอง_${room.name}.csv`);
